...
class SnakeGame:
    def __init__(self, seed=0, board_size=15, silent_mode=True):
        ...
        if not silent_mode:
            ...
            # Load sound effects
            mixer.init()
            self.sound_start = mixer.Sound("sound/start.mp3")
            self.sound_eat = mixer.Sound("sound/eat.mp3")
            self.sound_game_over = mixer.Sound("sound/game_over.mp3")
    ...
    def step(self, action):
        ...
        if (row, col) == self.food: # If snake eats food, it won't pop the last cell. The food grid will be taken by snake later, no need to update board vacancy matrix.
            food_obtained = True
            self.score += 1 # Add 1 points to the score when food is eaten.
    ...
    def draw_welcome_screen(self):
        title_text = self.font.render("POSE_ESTIMATION_SNAKE GAME", True, (255, 0, 0))
        start_button_text = "START THE GAME"
    ...
    def draw_game_over_screen(self):
        ...
        final_score_text = self.font.render(f"SCORE: {self.score}", True, (255, 0, 0))
    ...
    def render(self):
        ...
        # Draw border
        pygame.draw.rect(self.screen, (0, 0, 255), (self.border_size - 2, self.border_size - 2, self.width + 4, self.height + 4), 2)
        ...
        if len(self.snake) < self.grid_size: # If the snake occupies the entire board, don't draw food.
            r, c = self.food
            food = pygame.image.load("image/ham.png")
            player_rect = food.get_rect(center = ((c * self.cell_size + self.border_size+20, r * self.cell_size + self.border_size+20 )))
            self.screen.blit(food,player_rect)
    ...
    def draw_snake(self):
        ...
        # Draw the head
        snake = pygame.image.load("image/snake_d.png")
        '''
        if action ==0:
            snake = pygame.image.load("image/snake_u.png")
        elif action ==2:
            snake = pygame.image.load("image/snake_l.png")
        elif action ==3:
            snake = pygame.image.load("image/snake_r.png")'''
            
        player_rect = snake.get_rect(center = ((head_x +10 + self.border_size // 2 , head_y +20 + self.border_size//2  )))
        if action ==2 :
            player_rect = snake.get_rect(center = ((head_x  + self.border_size // 2 , head_y +10 + self.border_size//2)))
        elif action==3:
            player_rect = snake.get_rect(center = ((head_x+20  + self.border_size // 2 , head_y +10 + self.border_size//2)))
        elif action==0:
            player_rect = snake.get_rect(center = ((head_x+10  + self.border_size // 2 , head_y  + self.border_size//2))) 
        self.screen.blit(snake,player_rect)
        '''
        ...
        pygame.draw.circle(self.screen, (255, 0, 0), (head_x + eye_offset, head_y + 3*eye_offset), eye_size)
        pygame.draw.circle(self.screen, (255, 0, 0), (head_x + self.cell_size - eye_offset, head_y + 3*eye_offset), eye_size)
'       '''
        # Draw the body (color gradient)
        color_list = np.linspace(255, 0, len(self.snake), dtype=np.uint8)
        i = 1
        for r, c in self.snake[1:]:
            ...
            body_radius = 5
            pygame.draw.rect(self.screen, (100, color_list[i], 0),
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("POSE_ESTIMATION_SNAKE GAME")
    game.font = pygame.font.Font(None, 50)
    

    game_state = "welcome"

    # Two hidden button for start and retry click detection
    start_button = game.font.render("START", True, (0, 255, 0))
    retry_button = game.font.render("RETRY", True, (0, 255, 0))
